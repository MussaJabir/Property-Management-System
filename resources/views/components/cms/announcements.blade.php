...
<section class="space-y-3">
    @forelse ($items as $item)
        <article class="rounded-xl bg-white p-6 shadow-sm dark:bg-zinc-900 dark:ring-1 dark:ring-white/10">
            ...
            @if ($item->excerpt)
                <p class="mt-2 text-sm text-zinc-600 dark:text-zinc-300">{{ $item->excerpt }}</p>
            @endif
            <div class="prose prose-zinc mt-3 max-w-none whitespace-pre-line text-sm dark:prose-invert">
                {{ $item->body }}
            </div>
        </article>
    @empty
